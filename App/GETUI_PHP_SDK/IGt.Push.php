...
use App\GETUI_PHP_SDK\igetui\utils\GTConfig;
use App\GETUI_PHP_SDK\igetui\utils\HttpManager;
use App\GETUI_PHP_SDK\igetui\utils\ApiUrlRespectUtils;
Class IGeTui
{
    var $appkey; //第三方 标识
...
//        $res = $this->httpPostJSON($this->host,$params);

        return $this->httpPostJSON($this->host,$params);
    }

    /**
     * @param $message IGtSingleMessage
     * @param $target IGtTarget
...
//        print_r($message->get_data());
        file_put_contents('./log.log', '1');
        /**
         * var $message->get_data() IGtNotyPopLoadTemplate
         */
        $params["clientData"] = base64_encode($message->get_data()->get_transparent());
        file_put_contents('./log.log', '1', FILE_APPEND);
