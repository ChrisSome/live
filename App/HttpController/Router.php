...
class Router extends AbstractRouter
{
	public function initialize(RouteCollector $routeCollector)
	{
		...
		$routeCollector->addGroup('/api', function (RouteCollector $r) {
			...
			$r->addRoute(['POST'], '/user/upload', '/User/Upload/index'); //普通上传
			$r->addRoute(['POST'], '/user/ossUpload', '/User/Upload/ossUpload'); //OSS上传
			$r->addRoute(['POST'], '/user/doLogin', '/User/Login/userLogin'); //用户登录
			$r->addRoute(['POST'], '/user/wxLogin', '/User/Login/wxLogin'); //微信登录
