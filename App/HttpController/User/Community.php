...
class Community extends FrontUserController
{
	...
	public function getPostChildComments(): bool
	{
		...
		$model = $model->where('parent_id', $commentId)
			->where('post_id', $postId)->where('status', 1)
			->order('created_at', 'DESC')
			->getAll($page, $size);
		$return['data'] = $model->all();
		$return['count'] = $model->lastQueryResult()->getTotalCount();
	...
	public function getAllChildComments(): bool
	{
		...
		// 获取评论信息
		$comment = AdminPostComment::getInstance()->find($commentId);
		if (empty($comment)) {
	...
	public function commentInfo(): bool
	{
		...
		return $this->writeJson(Status::CODE_OK, Status::$msg[Status::CODE_OK], $comment);
	}
	...
	public function getContentByKeyWord(): bool
	{
		...
		if ($type == 1 || $type == 2) {
			...
			$model = AdminUserPost::getInstance()
				->where('status', $statusArr, 'in')->where('title', '%' . $keywords . '%', 'like')->getLimit($page, $size);
			$list = $result['format_posts']['list'] = FrontService::handPosts($model->all(), $authId);
			$count = $result['format_posts']['count'] = $model->lastQueryResult()->getTotalCount();
			if ($type == 2) {
		...
		if ($type == 1 || $type == 3) {
			$model = AdminInformation::getInstance()
				->where('status', AdminInformation::STATUS_NORMAL)
				->where('title', '%' . $keywords . '%', 'like')->getLimit($page, $size);
			$list = $result['information']['list'] = FrontService::handInformation($model->all(), $authId);
			$count = $result['information']['count'] = $model->lastQueryResult()->getTotalCount();
			if ($type == 3) {
				$result = ['data' => $list, 'count' => $count];
				return $this->writeJson(Status::CODE_OK, Status::$msg[Status::CODE_OK], $result);
			}
		...
		if ($type == 1 || $type == 4) {
			$list = [];
			$count = 0;
			$team = AdminTeam::getInstance()->where('name_zh', '%' . $keywords . '%', 'like')->all();
			if (!empty($team)) {
				$teamIds = array_column($team, 'team_id');
				if (!empty($teamIds)) {
					$teamIdsStr = join(',', $teamIds);
					$model = AdminMatch::getInstance()
						->where('home_team_id in(' . $teamIdsStr . ') or away_team_id in(' . $teamIdsStr . ')')->getLimit($page, $size);
					$list = $result['format_matches']['list'] = FrontService::handMatch($model->all(), 0, true, false, true);
					$count = $result['format_matches']['count'] = $model->lastQueryResult()->getTotalCount();
				}
			}
			if ($type == 4) {
				$result = ['data' => $list, 'count' => $count];
		...
		if ($type == 1 || $type == 5) {
			...
			$model = AdminUser::getInstance()
				->where('nickname', '%' . $keywords . '%', 'like')->where('status', $statusArr, 'in')->getLimit($page, $size);
			$list = $result['users']['list'] = FrontService::handUser($model->all(), $authId);
			$count = $result['users']['count'] = $model->lastQueryResult()->getTotalCount();
			if ($type == 5) {
				$result = ['data' => $list, 'count' => $count];
				return $this->writeJson(Status::CODE_OK, Status::$msg[Status::CODE_OK], $result);
			}
	...
	public function myFollowUserPosts(): bool
	{
		...
		$model = AdminUserPost::getInstance()
			...
			->where('status', AdminUserPost::STATUS_EXAMINE_SUCC)->where('user_id', $userIds, 'in')
			->getLimit($page, $size);
		$list = $model->all();
		// 输出数据
		$result = [
	...
	public function postAdd(): bool
	{
		...
		if ($isPublish) {
			...
			if (!empty($words)) {
				foreach ($words as $v) {
					...
					if (strstr($params['content'], $v['word']) || strstr($params['title'], $v['word'])) {
						// 帖子已保存,不再发送站内信
						if ($postId > 0) {
							return $this->writeJson(Status::CODE_ADD_POST_SENSITIVE, sprintf(Status::$msg[Status::CODE_ADD_POST_SENSITIVE], $v['word']));
						}
						//发送站内信
			...
			return $this->writeJson(Status::CODE_OK, Status::$msg[Status::CODE_OK], $post);
		}
		// 保存
		$data['status'] = AdminUserPost::NEW_STATUS_SAVE;
	...
	public function userInfo(): bool
	{
		...
		return $this->writeJson(Status::CODE_OK, Status::$msg[Status::CODE_OK], $userInfo);
	}
