...
class Community extends FrontUserController
{
	...
	public function hotSearch()
	{
		...
		// 输出数据
		$hotSearch = empty($mapper[AdminSysSettings::SETTING_HOT_SEARCH]) ? [] : json_decode($mapper[AdminSysSettings::SETTING_HOT_SEARCH], true);
		$defaultSearchContent = empty($mapper[AdminSysSettings::SETTING_HOT_SEARCH_CONTENT]) ? '' : $mapper[AdminSysSettings::SETTING_HOT_SEARCH_CONTENT];
	...
	public function userInfo()
	{
		...
		$postCount = AdminUserPost::getInstance()->findOne(['user_id' => $userId, 'status' => AdminUserPost::NEW_STATUS_NORMAL], 'count');
		$postCommentCount = AdminUserPost::getInstance()->findOne(['user_id' => $userId, 'status' => [AdminPostComment::STATUS_DEL, '<>']], 'count');
	...
	{
		// 获取清单
		$result = AdminNormalProblems::getInstance()->findAll(['status' => 1, 'formal_time' => [date('Y-m-d H:i:s'), '<=']]);
		$this->output(Status::CODE_OK, Status::$msg[Status::CODE_OK], $result);
	}
