...
use App\Model\AdminSystemAnnouncement;
...
class System extends FrontUserController
{
	...
	public function detail()
	{
		...
		if (!$info) {
			$this->output(Status::CODE_ERR, '对应公告不存在');
		}
		//写异步task记录已读
		$auth = $this->auth;
	...
	public function advertisement()
	{
		...
		$ads = AdminAdvertisement::getInstance()
			->where('status', AdminAdvertisement::STATUS_NORMAL)
			->where('cat_id', $this->params['cat_id'])->all();
		$this->output(Status::CODE_OK, Status::$msg[Status::CODE_OK], $ads);
	}
	
	public function sensitiveWord()
	{
		$words = AdminSensitive::getInstance()->where('id', 0, '>')->field(['word'])->all();
		$this->output(Status::CODE_OK, Status::$msg[Status::CODE_OK], $words);
	}
}