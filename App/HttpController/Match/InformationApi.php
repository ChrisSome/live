...
class InformationApi extends FrontUserController
{
	...
	public function getCategoryInformation()
	{
		...
		if ($type == 1) {
			...
			if ($competition_id = $this->params['competition_id']) {
				...
				$this->output(Status::CODE_OK, Status::$msg[Status::CODE_OK], $title_content);
			} else {
	...
	public function informationInfo()
	{
		...
		if (intval($information['status']) == AdminInformation::STATUS_DELETE) $this->output(Status::CODE_WRONG_RES, Status::$msg[Status::CODE_WRONG_RES]);
		// 填充用户信息
	...
	public function informationComment()
	{
		...
		if ($sensitiveWords = AdminSensitive::getInstance()->where('status', AdminSensitive::STATUS_NORMAL)->field(['word'])->all()) {
			foreach ($sensitiveWords as $sword) {
				if (strstr($this->params['content'], $sword['word'])) {
					$this->output(Status::CODE_ADD_POST_SENSITIVE, sprintf(Status::$msg[Status::CODE_ADD_POST_SENSITIVE], $sword['word']));
				}
			}
		}
		if (!$information = AdminInformation::getInstance()->findOne($information_id)) {
			$this->output(Status::CODE_W_PARAM, Status::$msg[Status::CODE_W_PARAM]);
		} else {
			$data = [
				'information_id' => $this->params['information_id'],
		...
		$this->output(Status::CODE_OK, Status::$msg[Status::CODE_OK], $comment_info_format);
	}
	...
	public function informationChildComment()
	{
		...
		if (!$father_comment = AdminInformationComment::getInstance()->findOne($top_comment_id)) {
			$this->output(Status::CODE_W_PARAM, Status::$msg[Status::CODE_W_PARAM]);
		}
