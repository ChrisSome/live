...
class PasswordTool
{
	...
	{
		if (empty($cost)) $cost = $this->passwordHashCost;
		if (function_exists('password_hash')) return password_hash($password, PASSWORD_DEFAULT, ['cost' => $cost]);
		$salt = $this->generateSalt($cost);
		$hash = crypt($password, $salt);
		if (!is_string($hash) || strlen($hash) !== 60) throw new \Exception('Unknown error occurred while generating hash.');
		return $hash;
	}
